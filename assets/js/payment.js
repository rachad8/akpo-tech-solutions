...
const FacturationAPI = {
    ...
    async payerFacture(facture) {
        if (!facture || !facture.id) throw new Error('Facture invalide.');
        if (facture.statut === 'payee') throw new Error('Cette facture est déjà payée.');
        if (!window.firebase || !firebase.functions) throw new Error('Firebase Functions n’est pas disponible.');

        await this.loadFedaPayCheckout();

        const createPayment = firebase.functions().httpsCallable('createFedaPayTransaction');
        const result = await createPayment({ factureId: facture.id });
        const payment = result.data;

        if (!payment?.success || !payment.transactionId) throw new Error('Impossible de créer la transaction FedaPay.');
        if (!FEDAPAY_CONFIG.publicKey || FEDAPAY_CONFIG.publicKey.startsWith('VOTRE_')) {
            throw new Error('La clé publique FedaPay n’est pas encore configurée dans assets/js/payment.js.');
        }
        ...
        const widget = FedaPay.init({
            ...
            transaction: {
                ...
                description: 'Paiement facture ' + facture.id
            },
            customer: {
                email: facture.clientEmail || '',
                lastname: facture.clientNom || 'Client'
            },
            onComplete: async (reason, transaction) => {
                if (reason !== FedaPay.CHECKOUT_COMPLETED) return;
                try {
                    const verifyPayment = firebase.functions().httpsCallable('verifyFedaPayPayment');
                    const verification = await verifyPayment({
                        factureId: facture.id,
                        transactionId: transaction?.id || payment.transactionId
                    });
                    if (verification.data?.paid) {
                        alert('Paiement confirmé. Votre facture est maintenant payée.');
                        window.location.reload();
                    } else {
                        alert('Le paiement n’a pas encore été confirmé. Vérifiez votre facture dans quelques instants.');
                    }
                } catch (error) {
                    console.error('Erreur de vérification FedaPay:', error);
                    alert('La confirmation automatique du paiement a échoué. Actualisez la page ou contactez le support.');
                }
            }
        });
    ...
    loadFedaPayCheckout() {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (existing) {
                ...
                existing.addEventListener('error', () => reject(new Error('Impossible de charger FedaPay Checkout.')), { once: true });
                return;
            }
            ...
            script.onerror = () => reject(new Error('Impossible de charger FedaPay Checkout.'));
    ...
    generateFactureHTML(facture) {
        ...
        return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
            ...
            .header{text-align:center;margin-bottom:30px;}.header h1{color:#3B82F6;margin:0;font-size:24px;}.header p{color:#64748B;margin:5px 0;font-size:12px;}
            .facture-title{text-align:center;font-size:20px;font-weight:bold;margin:20px 0;}.info-table{width:100%;margin-bottom:20px;}.info-table td{padding:4px 0;font-size:13px;}
            .table{width:100%;border-collapse:collapse;margin:20px 0;}.table th{background:#3B82F6;color:white;padding:10px;text-align:left;font-size:13px;}.table td{padding:10px;border-bottom:1px solid #E2E8F0;font-size:13px;}
            ...
            <table class="info-table"><tr><td style="color:#64748B;width:120px;">Date :</td><td>${date}</td></tr><tr><td style="color:#64748B;">Client :</td><td>${facture.clientNom}</td></tr><tr><td style="color:#64748B;">Email :</td><td>${facture.clientEmail}</td></tr><tr><td style="color:#64748B;">Téléphone :</td><td>${facture.clientTelephone||'-'}</td></tr><tr><td style="color:#64748B;">Statut :</td><td><span class="statut ${facture.statut==='payee'?'statut-payee':'statut-attente'}">${facture.statut==='payee'?'Payée':'En attente'}</span></td></tr></table>
            ...
            <div class="total">TOTAL : ${facture.montant.toLocaleString()} FCFA</div><div class="footer"><p>AKPO TECH SOLUTIONS - Votre informatique, notre priorité</p><p>Merci de votre confiance !</p></div>
        </body></html>`;
    },

    downloadFacturePDF(facture) {
        const printWindow = window.open('', '_blank');
        printWindow.document.write(this.generateFactureHTML(facture));
