...
const FacturationAPI = {
    ...
    async createFacture(data) {
        try { const factureId = this.generateFactureId(); const facture = { id: factureId, clientId: data.clientId, clientNom: data.clientNom, clientEmail: data.clientEmail, clientTelephone: data.clientTelephone || '', demandeId: data.demandeId || '', service: data.service, description: data.description || '', montant: data.montant, date: firebase.firestore.FieldValue.serverTimestamp(), statut: 'en-attente' }; await db.collection('factures').doc(factureId).set(facture); return { success: true, data: facture }; }
        catch (error) { return { success: false, error: error.message }; }
    },
    async getClientFactures(clientId) { try { const snapshot = await db.collection('factures').where('clientId', '==', clientId).orderBy('date', 'desc').get(); const factures = []; snapshot.forEach(doc => factures.push({ id: doc.id, ...doc.data() })); return { success: true, data: factures }; } catch (error) { return { success: false, error: error.message }; } },
    async getAllFactures() { try { const snapshot = await db.collection('factures').orderBy('date', 'desc').get(); const factures = []; snapshot.forEach(doc => factures.push({ id: doc.id, ...doc.data() })); return { success: true, data: factures }; } catch (error) { return { success: false, error: error.message }; } },
    async updateStatut(factureId, statut) { try { await db.collection('factures').doc(factureId).update({ statut, datePaiement: statut === 'payee' ? firebase.firestore.FieldValue.serverTimestamp() : null }); return { success: true }; } catch (error) { return { success: false, error: error.message }; } },
    async deleteFacture(factureId) { try { await db.collection('factures').doc(factureId).delete(); return { success: true }; } catch (error) { return { success: false, error: error.message }; } },
    ...
    async payerFactureById(factureId) {
        ...
        await this.payerFacture({ id: snap.id, ...snap.data() });
    },
    loadFedaPayCheckout() {
...
(function initFedaPayInvoiceButtons() {
    function injectButtons() {
        ...
        table.querySelectorAll('tr').forEach(row => {
            ...
            const factureId = strong.textContent.trim(); const status = row.querySelector('td:nth-child(4)')?.textContent.trim().toLowerCase();
            if (!factureId || status === 'payee') return;
            const actions = row.querySelector('td:last-child'); if (!actions) return;
            const button = document.createElement('button'); button.className = 'btn btn-sm btn-success btn-action btn-fedapay-pay'; button.title = 'Payer avec FedaPay'; button.innerHTML = '<i class="bi bi-credit-card"></i> Payer';
            button.addEventListener('click', async () => { button.disabled = true; try { await FacturationAPI.payerFactureById(factureId); } catch (error) { console.error(error); alert(error.message || 'Impossible de lancer le paiement.'); button.disabled = false; } });
